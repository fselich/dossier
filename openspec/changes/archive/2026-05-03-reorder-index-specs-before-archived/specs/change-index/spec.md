...
The TUI SHALL implement a `ModeIndex` mode that occupies the entire screen with the same TUI chrome (borders, header, helpbar). The index SHALL display three sections: "Active Changes" with the active changes, "Specifications" with the project specs in `openspec/specs/`, and "Archived Changes" with the changes in `openspec/changes/archive/`; all three separated by a section divider line.
